...
export class ColeccionFunkos {
  ...
  constructor(nombreUsuario: string) {
    ...
    if (!existsSync('./libreriaFunkos/' + this._nombreUsuario)) { // ¿existe el directorio del usuario?
      mkdirSync('./libreriaFunkos/' + nombreUsuario);

      
    } else {
      // Almacenamos los datos
      let registrosFunkos = readdirSync('./libreriaFunkos/' + this._nombreUsuario);

      for (let i : number = 0; i < registrosFunkos.length; i++) {
        let funko = readFileSync('./libreriaFunkos/' + this._nombreUsuario + '/' + registrosFunkos[i], 'utf8'); // valor predeterminado
  ...
  listar () : void {
    this._funkos.forEach(funko => {
      ...
      console.log(`Características especiales: ${funko._caracteristicasEspeciales}`);
