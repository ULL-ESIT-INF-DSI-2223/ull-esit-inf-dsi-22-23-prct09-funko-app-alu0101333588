...
yargs(hideBin(process.argv))
  .command(
    ...
    (argv) => {
      if (!existsSync("./libreriaFunkos/" + argv.nombreUsuario)) {
        // ¿existe el directorio del usuario?
        mkdirSync("./libreriaFunkos/" + argv.nombreUsuario); // Creamos un fichero con el nick del usuario
        console.log(
          chalk.green.bold(
            `El usuario ${argv.nombreUsuario} se ha creado correctamente`
          )
        );
      } else {
        console.log(
          chalk.red.bold(`Error. El usuario ${argv.nombreUsuario} ya existe`)
        );
      }
    }
  )
  ...
  .command(
    ...
    (argv) => {
      let coleccionFunko1: ColeccionFunkos = new ColeccionFunkos(
        argv.nombreUsuario
      );
